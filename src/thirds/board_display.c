...
{
	gtk_widget_set_sensitive(home_button_undo, current_game->parent != NULL);
	gtk_widget_set_sensitive(home_button_redo, has_children(current_game));
}
...
void open_pgn_callback(GtkWidget *widget, gpointer user_data)
{
...
void new_game_callback (GtkWidget *widget, gpointer user_data)
{
